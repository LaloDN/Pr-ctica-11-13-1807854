...
export class ReservacionService{
    ...
    fetchReservaciones(){
        let url: string ='https://proyecto-1807854.firebaseio.com/reservaciones.json?orderBy="usuarioId"&equalTo=${this.loginService.usuarioId}';
    ...
    addReservcaion(lugarId: number, descripcion:string, imageUrl: string, nombre: string, apellido: string, huespedes: number, desde: Date, hasta: Date){
        let firebaseId: string;
        const newReservacion = new Reservacion(Math.random()*100,lugarId, this.loginService.usuarioId, descripcion, imageUrl, nombre,apellido,huespedes,desde,hasta,null);
        return this.http.post<{name:string}>('https://proyecto-1807854.firebaseio.com/reservaciones.json',{...newReservacion,null}).pipe(switchMap(resData=>{
    ...
    cancelarReservacion(firebaseId: string){
        return this.http.delete('https://proyecto-1807854.firebaseio.com/reservaciones.json').pipe(swithcMap(()=>{
            return this.reservaciones;
        }),take(1), tap(rsvs=>{
